...
def area_parse(text, radius=40):
    ...
    if text.startswith(('http://', 'https://')):
        ...
        if u.hostname not in ('park4night.com', 'www.park4night.com','openstreetmap.org','www.openstreetmap.org'):
            raise ValueError('Obsługiwane URL mapy: Park4Night i OpenStreetMap; dla innych map użyj bbox.')
        ...
        if u.hostname in ('openstreetmap.org','www.openstreetmap.org'):
            ...
            if len(parts)==3:q['lat']=[parts[1]];q['lng']=[parts[2]]
            elif 'mlat' in q and 'mlon' in q:q['lat']=q['mlat'];q['lng']=q['mlon']
        try:
        ...
        return {'bbox': [max(-180, lon-dx), max(-90, lat-dy), min(180, lon+dx), min(90, lat+dy)],
                'center': [lat, lon], 'radius_km': r,
...
def valid_url(s, p4n=False):
    ...
    return u.scheme=='https' and bool(u.hostname) and not u.username and not u.password and (not p4n or u.hostname in ('park4night.com','www.park4night.com'))
...
def normalize(dataset):
    ...
    for index, raw in enumerate(dataset['spots']):
        if not isinstance(raw,dict): raise ValueError(f'Miejsce {index+1}: oczekiwano obiektu.')
        ...
        if not isinstance(p['source'],str): raise ValueError('source musi być tekstem.')
        link=p.get('park4night_url','')
        if link and not valid_url(link,True): raise ValueError('Nieprawidłowy link Park4Night.')
        ...
        for field in ('description','type'):
            p.setdefault(field,'')
            if not isinstance(p[field],str): raise ValueError(f'{field} musi być tekstem.')
        for field in ('comments','photos','evidence'):
            p.setdefault(field,[])
            if not isinstance(p[field],list): raise ValueError(f'{field} musi być listą.')
        for c in p['comments']:
            if not isinstance(c,dict) or not isinstance(c.get('text'),str): raise ValueError('Komentarz wymaga text.')
        for ev in p['evidence']:
            if not isinstance(ev,dict) or not isinstance(ev.get('text'),str): raise ValueError('Dowód wymaga text.')
        p.setdefault('geo',{})
        if not isinstance(p['geo'],dict): raise ValueError('geo musi być obiektem.')
        if p.get('source_url') and not valid_url(p['source_url']):raise ValueError('source_url musi być poprawnym HTTPS URL.')
        if p.get('extra_flags') is not None and (not isinstance(p['extra_flags'],list) or any(not isinstance(s,str) for s in p['extra_flags'])):raise ValueError('extra_flags musi być listą tekstów.')
        for field,low,high in [('elevation_m',-500,9000),('water_distance_m',0,1000000),('slope_deg',0,90)]:
            if p['geo'].get(field) is not None: p['geo'][field]=number(p['geo'][field],low,high,field)
        for photo in p['photos']:
            if not isinstance(photo,dict): raise ValueError('Zdjęcie musi być obiektem.')
            url=photo.get('url','')
...
def age(date):
    try: return (TODAY()-dt.date.fromisoformat(date)).days
...
def rank(p, ai=None):
    ...
    hints={'scenic':r'panoram|widok|view|aussicht|vue|grzbiet',
    ...
    if re.search(r'tłum|crowd|hałas|noisy|busy',text): scores['solitude']=20
    if geo.get('viewpoint'):
        scores['scenic']=70;why.append('Źródło mapowe oznacza punkt widokowy; nie potwierdza noclegu ani widoczności.')
    if geo.get('water_access_point'):
        scores['water']=70;why.append('Źródło mapowe oznacza punkt wodowania; nie potwierdza miejsca na namiot.')
    ...
    if geo.get('water_distance_m') is not None:
        d=geo['water_distance_m']; scores['water']=95 if d<=50 else 80 if d<=150 else 45 if d<=500 else 10
        why.append(f'Woda: {d:g} m według importu; odległość nie potwierdza zejścia do brzegu')
        if d<100: flags.append('Blisko wody: sprawdź wezbrania i bezpieczną wysokość namiotu')
    ...
    if geo.get('slope_deg',0)>12:
        scores['adv_access']=min(scores['adv_access'] or 40,40); flags.append('Stromy teren: wymaga sprawdzenia dojazdu i płaskiego miejsca')
    if surface in ('sand','mud','rock'):
        scores['adv_access']=20; flags.append('Trudna nawierzchnia dla załadowanego ADV')
    ...
    for k in permissions:
        ...
        if any(e.get('value')=='forbidden' for e in evs): permissions[k]='blocked'
        elif any(e.get('value')=='allowed' and e.get('authority') in ('owner','official') and
                 valid_url(e.get('source_url','')) and age(e.get('date')) is not None and 0<=age(e.get('date'))<=365 for e in evs):
            permissions[k]='supported'
    blocked=any(v=='blocked' for v in permissions.values())
    for k,label in [('motorcycle','wjazdu motocyklem'),('tent','rozbicia namiotu'),('adjacent','namiotu obok motocykla')]:
        if permissions[k]=='unknown': flags.append(f'Brak aktualnego potwierdzenia: {label}')
        if permissions[k]=='blocked': flags.append(f'Informacja o zakazie / niemożliwości: {label}')
    restrictive=re.search(r'zakaz|szlaban|no camping|no tents|no motor|barrier|verbot|interdit|private|prywatn',text)
    if restrictive: flags.append('Tekst zawiera wzmiankę o ograniczeniach — sprawdź kontekst i sprzeczności')
    if geo.get('protected_area'): flags.append('Obszar chroniony: sprawdź regulamin i wyznaczone miejsca')
    if geo.get('forest'): flags.append('Las: przejezdna droga nie oznacza dopuszczenia motocykli')
    if any(age(x.get('date')) is not None and age(x.get('date'))>365 for x in items): flags.append('Część informacji ma ponad rok')
    if not p['comments']: flags.append('Brak komentarzy')
    if not p['photos']: flags.append('Brak zdjęć')
    flags.extend(p.get('extra_flags') or [])
    enough=all(v=='supported' for v in permissions.values())
    status='excluded' if blocked or (scores['adv_access'] is not None and scores['adv_access']<35) else 'candidate' if enough and not restrictive and not geo.get('protected_area') else 'verify'
    if ai and ai.get('red_flags') and status=='candidate': status='verify'
    legal=0 if blocked else round(sum(v=='supported' for v in permissions.values())/3*90)
    if restrictive or geo.get('protected_area'): legal=min(legal,30)
    ...
    A=weighted(dict(adv_access=.25,scenic=.50,water=0,solitude=.25))
    B=weighted(dict(adv_access=.25,scenic=.10,water=.45,solitude=.20))
...
def analyze_ai(p, include_photos=False):
    ...
    if not key or not model: raise ValueError('Ustaw OPENAI_API_KEY i OPENAI_MODEL na serwerze.')
    ...
    schema={'type':'object','additionalProperties':False,'properties':{
        'scores':{'type':'object','additionalProperties':False,'properties':{k:numeric for k in METRICS},'required':list(METRICS)},
        'reasons':{'type':'array','items':{'type':'string'}},'red_flags':{'type':'array','items':{'type':'string'}},
        'evidence_ids':{'type':'array','items':{'type':'string'}},
        'photo_notes':{'type':'array','items':{'type':'string'}}},
        'required':['scores','reasons','red_flags','evidence_ids','photo_notes']}
    content=[{'type':'input_text','text':json.dumps({'name':p['name'],'type':p['type'],'facts':facts(p),'geo':p['geo'],'photos':[{'id':f'photo:{i}','caption':x.get('caption','')} for i,x in enumerate(images)]},ensure_ascii=False)}]
    content += [{'type':'input_image','image_url':x['url'],'detail':'low'} for x in images]
    payload={'model':model,'store':False,'instructions':
        'Oceniasz miejsca na namiot obok motocykla ADV, lekki off-road, południowa Polska. '
        ...
        'text':{'format':{'type':'json_schema','name':'adv_assessment','strict':True,'schema':schema}},'max_output_tokens':1800}
    req=urllib.request.Request('https://api.openai.com/v1/responses',json.dumps(payload).encode(),
          {'Authorization':'Bearer '+key,'Content-Type':'application/json'})
    with urllib.request.urlopen(req,timeout=55) as response: raw=json.load(response)
    if raw.get('status')!='completed': raise ValueError('AI nie ukończyło odpowiedzi; zachowano ocenę lokalną.')
    out=''.join(c.get('text','') for m in raw.get('output',[]) for c in m.get('content',[]) if c.get('type')=='output_text')
    a=json.loads(out)
    if not isinstance(a,dict) or not isinstance(a.get('scores'),dict): raise ValueError('Nieprawidłowa odpowiedź AI.')
    for k in METRICS:
        if k not in a['scores']: raise ValueError('AI pominęło ocenę.')
        if a['scores'][k] is not None: a['scores'][k]=number(a['scores'][k],0,100,k)
    for k in ('reasons','red_flags','evidence_ids','photo_notes'):
        if not isinstance(a.get(k),list) or any(not isinstance(s,str) for s in a[k]): raise ValueError('Nieprawidłowe uzasadnienie AI.')
    allowed={x['id'] for x in facts(p)} | {'geo'} | {f'photo:{i}' for i in range(len(images))}
    if not set(a['evidence_ids']) <= allowed: raise ValueError('AI wskazało nieistniejący dowód.')
    if not a['evidence_ids'] and any(v is not None for v in a['scores'].values()): raise ValueError('AI nie wskazało dowodów ocen.')
    if not images: a['photo_notes']=[]
    a['photos_analyzed']=len(images); a['model']=model; a['date']=str(TODAY())
    return a
