...
class VisionTests(unittest.TestCase):
    def test_block_untrusted_image_hosts(self):
        for url in [
            ...
            f'https://{CDN3}.evil.test/a.jpg',
            f'https://user:pass@{CDN3}/a.jpg',
            f'https://{CDN3}:444/a.jpg',
        ]:
    ...
    def test_reject_non_image_payload(self):
        with self.assertRaises(ValueError):
            v.image_bytes('data:image/png;base64,aGVsbG8=')
    ...
    def test_score_validation(self):
        ...
        for score in [True, float('nan'), 101, -1, '80']:
            with self.subTest(score=score), self.assertRaises(ValueError):
                v.validate(dict(valid, match=score))

    def test_fingerprint_ignores_review_but_tracks_source(self):
        p = dict(name='Test', photos=[{'url': 'one'}])
        h = v.fingerprint(p)
    ...
    def test_service_unavailable(self):
        with patch.object(v, 'local_call', side_effect=TimeoutError):
            self.assertFalse(v.status()['ready'])
