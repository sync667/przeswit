...
from app.core import TODAY, analyze_ai, area_parse, inside, normalize, rank
from app.p4n.endpoints import HOST as P4N
...
class Tests(unittest.TestCase):
    def test_bbox_and_circle(self):
        ...
        self.assertFalse(inside(spot(lat=51), area_parse(P4N + '/en/search?lat=50.9&lng=16.3&z=9', 1)))
        self.assertTrue(inside(spot(), area_parse(P4N + '/en/search?lat=50.9&lng=16.3&z=9', 1)))
    ...
    def test_stale_and_community_do_not_authorize(self):
        for ev in [
            permission('tent', date='2020-01-01'),
            permission('tent', authority='community'),
            permission('tent', date='2999-01-01'),
        ]:
    ...
    def test_dedup(self):
        p = spot()
        self.assertEqual(normalize({'spots': [p, copy.deepcopy(p)]})[1], 1)
    ...
    def test_link_validation(self):
        with self.assertRaises(ValueError):
            spot(p4n_url=P4N + '.evil.test/x')
        with self.assertRaises(ValueError):
            spot(photos=[{'url': 'javascript:alert(1)'}])

    def test_bad_number(self):
        with self.assertRaises(ValueError):
            spot(lat=True)
        with self.assertRaises(ValueError):
            spot(geo={'water_distance_m': -1})

    def test_missing_scores_do_not_inflate(self):
        r = rank(spot())
        self.assertEqual(r['rank_a'], 0)
        self.assertEqual(r['evidence_coverage'], 0)

    def test_protected_requires_review(self):
        r = rank(
            spot(geo={'protected_area': True}, evidence=[permission(k) for k in ('motorcycle', 'tent', 'adjacent')])
        )
        self.assertEqual(r['status'], 'verify')

    def test_ai_request_and_validation(self):
        answer = dict(
            scores=dict(adv_access=None, scenic=70, water=None, solitude=None),
            reasons=['Opis wskazuje panoramę (description)'],
            red_flags=[],
