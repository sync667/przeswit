...
export function exportCSV(){const rows=[['name','latitude','longitude','source','url','type','match','criteria','ADV','scenic','water','solitude','legal_confidence','choice','note','license','fetched_at','flags'],...filtered().map(p=>[p.name,p.lat,p.lon,p.source,p.source_url||p.park4night_url,p.type,currentMatch(p)?.match,$('wish').value.trim(),p.scores.adv_access,p.scores.scenic,p.scores.water,p.scores.solitude,p.legal_confidence,p.choice,p.user_note,p.license,p.fetched_at,p.red_flags.join(' | ')])];download('\uFEFF'+rows.map(r=>r.map(csvCell).join(';')).join('\r\n'),'text/csv;charset=utf-8','przeswit-miejsca.csv');}
