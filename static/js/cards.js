...
export async function choose(p,choice){const previous={key:p.key,choice:p.choice||'',note:p.user_note||''};try{await api('/api/note',{key:p.key,choice,note:p.user_note||''});p.choice=choice;state.undoAction=previous;$('undo-text').textContent=choice==='rejected'?'Miejsce odrzucone':choice?'Zachowano do oceny':'Przywrócono do przeglądania';$('undo-toast').hidden=false;render();}catch(e){notify(e.message,true);}}
