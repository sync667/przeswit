...
function readAsBase64(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result).split(',')[1]);r.onerror=()=>reject(Error(t('Błąd odczytu pliku.')));r.readAsDataURL(file);});}

export async function importFile(e){const f=e.target.files[0];if(!f)return;try{if(f.size>MAX_UPLOAD_BYTES)throw Error(t('Plik przekracza 20 MB.'));const data=await readAsBase64(f);const result=await api('/api/import',{name:f.name,content:data,source:$('file-source').value});await loadLibrary();notify(t('Import: {imported} rekordów, {saved} zapisanych/odświeżonych. Zachowano notatki.',{imported:result.imported,saved:result.saved}));}catch(err){notify(err.message,true);}finally{e.target.value='';}}

export function exportJSON(){download(JSON.stringify({label:t('Prześwit · osobisty eksport wybranych rekordów'),exported_at:new Date().toISOString(),scope:'visible_filters',spots:filtered()},null,2),'application/json','przeswit-miejsca.json');}
