...
export const BASEMAPS={
  osm:{label:t('Mapa OSM'),make:()=>L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:OSM_ATTR})},
  satellite:{label:t('Satelita'),make:()=>L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{maxNativeZoom:19,maxZoom:20,attribution:ESRI_ATTR})},
  relief:{label:t('Satelita + rzeźba terenu (3D)'),make:()=>L.layerGroup([
...
function initForestLayer(){
 const shell=document.querySelector('.map-shell'),bar=el('div',undefined,'forest-toolbar'),label=el('label',undefined,'check'),toggle=el('input');toggle.type='checkbox';toggle.id='forest-overlay';toggle.checked=true;label.append(toggle,document.createTextNode(t('Zanocuj w lesie')));const status=el('span',t('Obszary BDL od zoomu {zoom}',{zoom:FOREST_MIN_ZOOM}),'forest-status');status.id='forest-status';status.setAttribute('role','status');bar.append(label,status);shell.prepend(bar);
 const note=el('div',undefined,'forest-note');note.append(el('span','▧ '+t('Obszary programu')+' · '),link(t('Źródło: Bank Danych o Lasach ↗'),'https://www.bdl.lasy.gov.pl/portal/mapy'),el('div',t('Sprawdź regulamin nadleśnictwa i aktualne zakazy. Obszar programu nie oznacza zgody na wjazd motocyklem.')));shell.append(note);
 ...
 let failed=false;state.forestLayer.on('loading',()=>{failed=false;status.textContent=t('Wczytuję obszary BDL…');});state.forestLayer.on('tileerror',()=>{failed=true;status.textContent=t('BDL niedostępne — granice mogą być niepełne');});state.forestLayer.on('load',()=>{if(!failed)status.textContent=t('Obszary BDL · warstwa online');});toggle.onchange=()=>{if(toggle.checked&&state.mapView)state.forestLayer.addTo(state.map);else state.forestLayer.remove();status.textContent=toggle.checked?t('Obszary BDL od zoomu {zoom}',{zoom:FOREST_MIN_ZOOM}):t('Warstwa wyłączona');};
}

export function initMap(){if(!window.L){notify(t('Mapa niedostępna. Tabela nadal działa.'),true);return;}state.map=L.map('map',{preferCanvas:true}).setView([50.9247,16.2726],9);state.layer=L.layerGroup().addTo(state.map);initForestLayer();state.tiles=attachBasemaps(state.map,'list','osm',{[t('Zanocuj w lesie (BDL)')]:state.forestLayer},(key,layer)=>{state.tiles=layer;});if(!$('tiles').checked)state.tiles.remove();state.map.on('moveend',()=>{if($('only-map').checked){state.page=0;render(false);}});initLasso();}
