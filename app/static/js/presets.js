...
const BUILTIN={
 [t('Weekend przy trasie')]:{'photos-only':true,'route-only':true,'route-range':'3'},
 [t('Widoki i woda')]:{'photos-only':true,'type-filter':'water'},
 [t('Punkty widokowe')]:{'photos-only':true,'type-filter':'viewpoint'},
 [t('Tylko wyniki AI')]:{'ai-only':true},
 [t('Wszystko')]:{'photos-only':false,'type-filter':'','source-filter':'','search':'','route-only':false,'ai-only':false},
};
...
export function initPresets(){
 ...
 const sel=el('select');sel.id='preset';sel.setAttribute('aria-label',t('Preset filtrów'));
 const save=el('button',t('Zapisz preset'),'ghost');save.title=t('Zapisz bieżące filtry jako własny preset');
 const del=el('button','✕','ghost');del.title=t('Usuń wybrany własny preset');del.hidden=true;
 const nameBox=el('span',undefined,'preset-name');nameBox.hidden=true;const nameInput=el('input');nameInput.type='text';nameInput.placeholder=t('nazwa presetu');nameInput.maxLength=40;const ok=el('button','OK');nameBox.append(nameInput,ok);
 function fill(){const cur=custom();sel.replaceChildren(new Option(t('Presety…'),''));for(const n of Object.keys(BUILTIN))sel.append(new Option(n,'b:'+n));for(const n of Object.keys(cur))sel.append(new Option(t('{name} (własny)',{name:n}),'c:'+n));}
 sel.onchange=()=>{const v=sel.value;del.hidden=!v.startsWith('c:');if(!v)return;const s=v.startsWith('b:')?BUILTIN[v.slice(2)]:custom()[v.slice(2)];if(s){apply(s);notify(t('Preset: {name}',{name:v.slice(2)}));}};
 save.onclick=()=>{nameBox.hidden=false;nameInput.focus();};
 ok.onclick=()=>{const n=nameInput.value.trim();if(!n)return;const all=custom();all[n]=snapshot();saveCustom(all);fill();sel.value='c:'+n;del.hidden=false;nameBox.hidden=true;nameInput.value='';notify(t('Zapisano preset „{name}”.',{name:n}));};
...
export function initShortcuts(){
 document.addEventListener('keydown',e=>{
  ...
  switch(e.key){
   ...
   case '?':notify(t('Skróty: J/K kafle · S zachowaj · X odrzuć · Enter karta · M mapa · / szukaj · D motyw'));break;
  }
 });
...
export function applyTheme(){let t2=null;try{t2=localStorage.getItem(THEME_KEY);}catch{}if(!t2)t2=matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';document.documentElement.dataset.theme=t2;const b=$('theme-toggle');if(b)b.textContent=t2==='dark'?'☀':'☾';}
export function toggleTheme(){const t2=document.documentElement.dataset.theme==='dark'?'light':'dark';try{localStorage.setItem(THEME_KEY,t2);}catch{}applyTheme();}
export function initTheme(){const b=el('button','☾','ghost');b.id='theme-toggle';b.title=t('Motyw jasny/ciemny (D)');b.onclick=toggleTheme;document.querySelector('.header-right')?.prepend(b);applyTheme();}
