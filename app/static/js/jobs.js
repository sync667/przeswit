...
export async function startSync(){try{$('sync').disabled=true;const sources=['osm','bdl','p4n'].filter(s=>$('source-'+s).checked);await api('/api/sync',{areas:$('areas').value,radius:$('radius').value,sources});state.allLibrary=false;notify('Import uruchomiony. Możesz przeglądać wcześniej zapisane miejsca.');await poll();}catch(e){$('sync').disabled=false;notify(e.message,true);}}
