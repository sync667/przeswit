...
function placeCard(p){
 ...
 const actions=el('div',undefined,'card-actions'),reject=el('button',p.choice==='rejected'?'↶ Przywróć':'× Odrzuć'),save=el('button',SAVED_CHOICES.includes(p.choice)?'✓ Zachowane':'＋ Zachowaj do oceny','save-place');reject.onclick=()=>choose(p,p.choice==='rejected'?'':'rejected');save.onclick=()=>choose(p,SAVED_CHOICES.includes(p.choice)?'':'shortlist');actions.append(reject,save);const match=currentMatch(p);if(match){body.append(el('div',`✦ Dopasowanie ${match.match}/100 · pokrycie ${match.coverage}%`,'match-score'));if(match.conflicts.length)body.append(el('p',match.conflicts[0],'card-warning'));}const prof=p.profile_info?.profile;body.append(el('p',prof?`Profil AI · ${prof.photos_analyzed} zdjęć · ${prof.observations.length} cech`:'Profil AI: '+(p.profile_info?.status==='error'?'błąd — do ponowienia':'w kolejce'),'hint card-profile'));body.append(actions);card.append(media,body);
 return card;
}
...
function markerPopup(p,marker){
 ...
 function refresh(){const saved=SAVED_CHOICES.includes(p.choice);save.textContent=saved?'✓ Zachowane':'＋ Zachowaj';save.classList.toggle('chosen',saved);reject.textContent=p.choice==='rejected'?'↶ Przywróć':'× Odrzuć';reject.classList.toggle('chosen',p.choice==='rejected');}
 ...
 const open=el('button','Otwórz kartę →','ghost');open.onclick=()=>showDetail(p.key);
...
function appendBatch(all,rows){
 ...
 state.shown=end;
 $('page-info').textContent=all.length?`Pokazano ${state.shown} z ${all.length}`:'';
...
export function render(updateMap=true){const all=filtered();state.page=0;state.shown=0;$('count').textContent=`${all.length} / ${state.places.length} rekordów`;const rows=$('cards');clearHighlight();rows.replaceChildren();
...
function updateAfterChoice(p){
 ...
 if(card&&!stillListed){
  ...
  $('page-info').textContent=all.length?`Pokazano ${Math.min(state.shown,all.length)} z ${all.length}`:'';
  $('count').textContent=`${all.length} / ${state.places.length} rekordów`;
  ...
  $('page-info').textContent=`Pokazano ${Math.min(state.shown,all.length)} z ${all.length}`;$('count').textContent=`${all.length} / ${state.places.length} rekordów`;
 }
...
export async function bulkChoose(places,choice){
 ...
 if(previous.length){state.undoAction=previous;const n=previous.length;showUndoToast(`${choice==='rejected'?'Odrzucono':choice?'Zachowano':'Przywrócono'} ${n} ${n===1?'miejsce':n<5?'miejsca':'miejsc'}`);}
 render();return done;
}
