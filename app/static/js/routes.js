...
export function parseGPX(text,fileName){
 ...
 if(doc.querySelector('parsererror'))throw Error(`${fileName}: to nie jest poprawny plik GPX/XML.`);
 ...
 if(!lines.length&&!waypoints.length)throw Error(`${fileName}: brak śladów, tras ani punktów.`);
...
export function exportPlanGPX(){
 ...
 for(const [i,{p,pos}] of items.entries()){const w=doc.createElementNS(ns,'wpt');w.setAttribute('lat',p.lat);w.setAttribute('lon',p.lon);add(w,'name',`${i+1}. ${p.name}`);add(w,'desc',`km ${(pos.alongM/1000).toFixed(0)} · ${formatDistance(pos.offM)}${p.user_note?' · '+p.user_note:''}`);add(w,'sym','Campground');root.append(w);}
...
export function renderPlan(){
 ...
 if(!items.length){list.append(el('p',`Brak zachowanych miejsc w zasięgu ${nearRouteRange()} km od trasy. Zachowaj miejsca (＋), a pojawią się tu w kolejności jazdy.`,'hint'));return;}
 days.forEach((day,n)=>{if(!day)return;const h=el('h4',`Dzień ${n} · km ${(n-1)*planKmPerDay()}–${n*planKmPerDay()}`);list.append(h);for(const {p,pos} of day){const row=el('div',undefined,'plan-item');const open=el('button',p.name,'route-name');open.onclick=()=>import('./detail.js').then(m=>m.showDetail(p.key));row.append(el('span',`km ${(pos.alongM/1000).toFixed(0)}`,'plan-km'),open,el('span',formatDistance(pos.offM),'route-distance'));list.append(row);}});
}
...
export function formatDistance(m){if(!Number.isFinite(m))return '';return m<1000?`${Math.round(m)} m od trasy`:`${(m/1000).toFixed(m<10000?1:0)} km od trasy`;}
...
function persist(){
 try{const data=JSON.stringify(state.routes.map(r=>({...r})));if(data.length>STORAGE_LIMIT){notify('Trasy GPX są zbyt duże, by je zapamiętać między odświeżeniami (limit ~4 MB); zostaną do zamknięcia strony.',true);localStorage.removeItem(STORAGE_KEY);return;}localStorage.setItem(STORAGE_KEY,data);}catch{}
}
...
function renderList(){
 ...
 for(const r of state.routes){
  ...
  const label=el('button',undefined,'route-name');label.append(el('strong',r.name),el('span',` · ${(r.lengthM/1000).toFixed(1)} km · ${r.lines.length} ${r.lines.length===1?'segment':r.lines.length<5?'segmenty':'segmentów'}${r.waypoints.length?' · '+r.waypoints.length+' pkt':''}`));label.title='Pokaż trasę na mapie';label.onclick=()=>fitRoute(r);
  const remove=el('button','✕','ghost route-remove');remove.setAttribute('aria-label','Usuń trasę '+r.name);remove.onclick=()=>removeRoute(r.id);
  item.append(swatch,label,remove);list.append(item);
 }
 $('route-filter').hidden=!state.routes.length;
 if(!state.routes.length)list.append(el('p','Wczytaj jeden lub więcej plików GPX, aby zobaczyć trasę i odległość miejsc od niej.','hint'));
}
...
export async function addRouteFiles(files){
 ...
 notify(`Wczytano ${added} ${added===1?'trasę':'tras'} GPX. Miejsca pokazują odległość od najbliższej trasy.`);
}
...
export function initRoutes(){
 ...
 const head=el('div',undefined,'route-head');head.append(el('strong','Planowana trasa (GPX)'));
 ...
 const pick=el('button','＋ Dodaj GPX','route-add');pick.onclick=()=>input.click();
 ...
 filter.append(only,document.createTextNode('Tylko miejsca do '),range,document.createTextNode(' km od trasy'));
 ...
 const planHead=el('div',undefined,'route-head');planHead.append(el('strong','Plan jazdy (zachowane miejsca wzdłuż trasy)'));
 const km=el('label',undefined,'check');const kmInput=el('input');kmInput.type='number';kmInput.id='plan-km';kmInput.min='20';kmInput.step='10';kmInput.value='250';km.append(document.createTextNode('km/dzień '),kmInput);
 const exp=el('button','GPX planu ↓','ghost');exp.onclick=exportPlanGPX;planHead.append(km,exp);
