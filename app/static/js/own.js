...
export const OWN_STATUS={planned:'Planowane',visited:'Byłem',someday:'Na kiedyś'};
...
function toBase64(blob){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result).split(',')[1]);r.onerror=()=>reject(Error('Błąd odczytu pliku.'));r.readAsDataURL(blob);});}

function renderPhotos(){
 const box=$('own-photos');box.replaceChildren();
 for(const ph of editingPhotos){const item=el('figure',undefined,'own-photo');const img=el('img');img.src=state.photoCache[ph.url]||ph.url;img.alt=ph.caption||'';const del=el('button','✕','ghost');del.type='button';del.title='Usuń zdjęcie';del.onclick=async()=>{try{await api('/api/places/own/photos/delete',{key:editingKey,url:ph.url});editingPhotos=editingPhotos.filter(x=>x.url!==ph.url);renderPhotos();}catch(e){$('own-error').textContent=e.message;}};item.append(img,del);box.append(item);}
 const pending=[...($('own-files').files||[])];if(pending.length)box.append(el('p',`Do wysłania po zapisie: ${pending.map(f=>f.name).join(', ')}`,'hint'));
}
...
function fill(p,coords){
 ...
 $('own-title').textContent=p?'Edytuj własny punkt':'Dodaj własny punkt';
...
function stopPick(){if(pickHandler&&state.map){state.map.off('click',pickHandler);state.map.getContainer().classList.remove('pick-active');}pickHandler=null;$('own-pick').textContent='Wskaż na mapie';}

function startPick(){
 if(!state.map){notify('Najpierw przełącz na widok „Mapa + lista”.',true);return;}
 if(pickHandler){stopPick();return;}
 $('own-dialog').close();
 if(!state.mapView)$('toggle-map').click();
 notify('Kliknij na mapie miejsce, które chcesz zapisać (Esc anuluje).');
 state.map.getContainer().classList.add('pick-active');$('own-pick').textContent='Anuluj wskazywanie';
...
async function save(){
 ...
 if(!$('own-name').value.trim()){$('own-error').textContent='Podaj nazwę.';return;}
 if(!Number.isFinite(lat)||!Number.isFinite(lon)){$('own-error').textContent='Podaj współrzędne: wpisz, wklej link albo wskaż na mapie.';return;}
 ...
 try{
  ...
  $('own-dialog').close();await loadLibrary();if(uploaded)notify(`Zapisano punkt i ${uploaded} ${uploaded===1?'zdjęcie':uploaded<5?'zdjęcia':'zdjęć'}.`);
  if(!state.places.some(p=>p.key===r.key))notify('Punkt zapisany, ale jest poza bieżącymi filtrami (np. „Tylko ze zdjęciami”).');
...
async function remove(){
 ...
 try{await api('/api/places/own/delete',{key:editingKey});$('own-dialog').close();if($('detail').open)$('detail').close();await loadLibrary();notify('Własny punkt usunięty.');}
 catch(e){$('own-error').textContent=e.message;}
}
...
export function initOwn(){
 ...
 $('own-coords').oninput=()=>{const c=parseCoordinates($('own-coords').value);if(c){$('own-lat').value=c.lat.toFixed(6);$('own-lon').value=c.lon.toFixed(6);$('own-error').textContent='';}else if($('own-coords').value.trim())$('own-error').textContent='Nie rozpoznaję współrzędnych w tym tekście.';};
 $('own-form').onsubmit=e=>{e.preventDefault();save();};
 $('own-files').onchange=async()=>{renderPhotos();if(!$('own-lat').value&&!$('own-lon').value){for(const f of [...($('own-files').files||[])]){const c=await exifCoordinates(f).catch(()=>null);if(c){$('own-lat').value=c.lat.toFixed(6);$('own-lon').value=c.lon.toFixed(6);$('own-error').textContent='';notify(`Współrzędne pobrane z EXIF zdjęcia ${f.name}.`);break;}}}};
}
