...
export async function loadLibrary(fit=false){try{const r=await api('/api/library',{...areaRequest(),save_area:!state.allLibrary});state.photoCache=r.photo_cache||{};state.places=r.spots;state.page=0;syncRoutes();$('total').textContent=t('{n} rekordów w lokalnej bazie',{n:r.total});const selectedSource=$('source-filter').value;$('source-filter').replaceChildren(new Option(t('Wszystkie źródła'),''));for(const s of [...new Set(state.places.map(p=>p.source))].sort())$('source-filter').append(new Option(providerName(s),s));$('source-filter').value=selectedSource;render();if(fit&&state.map){if(r.areas.length){const b=r.areas.map(a=>a.bbox);state.map.fitBounds([[Math.min(...b.map(x=>x[1])),Math.min(...b.map(x=>x[0]))],[Math.max(...b.map(x=>x[3])),Math.max(...b.map(x=>x[2]))]]);}else if(state.places.length)state.map.fitBounds(state.places.map(p=>[p.lat,p.lon]),{maxZoom:11});}if(state.selected&&$('detail').open)showDetail(state.selected);notify(t('{n} rekordów {scope} · {outside} poza obszarem. Dane zachowane lokalnie. Kompletność dotyczy raportu importu, nie wszystkich miejsc w terenie.',{n:r.spots.length,scope:state.allLibrary?t('w całej bazie'):t('w wybranym obszarze'),outside:r.outside}));}catch(e){notify(e.message,true);}}
