...
HOSTS = set(CDN_HOSTS) | {'upload.wikimedia.org'}
MAX_IMAGE = 6_000_000
...
def status():
    try:
        ...
        return dict(
            ...
            model=MODEL,
            message=MODEL + ' · gotowy na tym komputerze' if MODEL in names else 'Pobierz model: ollama pull ' + MODEL,
        )
    except Exception:
        return dict(ready=False, model=MODEL, message='Uruchom Ollamę, aby analizować zdjęcia lokalnie.')


def _fetch_image_bytes(url):
    if url.startswith(('data:image/jpeg;base64,', 'data:image/png;base64,', 'data:image/webp;base64,')):
        if len(url) > MAX_IMAGE * 1.4:
        ...
        with OPENER.open(
            urllib.request.Request(url, headers={'User-Agent': 'Przeswit/1.0 local photo review'}), timeout=20
        ) as r:
            if not r.headers.get('Content-Type', '').split(';')[0] in ('image/jpeg', 'image/png', 'image/webp'):
                raise ValueError('Adres nie zwrócił obrazu.')
...
def image_bytes(url):
    ...
    if url.startswith('own://'):
        ...
        return base64.b64encode(photo_cache.read_own(url)).decode()
    u = urllib.parse.urlparse(url)
    if u.scheme != 'https' or u.hostname not in HOSTS or u.username or u.password or u.port not in (None, 443):
        return _fetch_image_bytes(url)
    from . import photo_cache
...
def fingerprint(p):
    return hashlib.sha256(
        json.dumps({k: p.get(k) for k in ('name', 'description', 'comments', 'photos', 'geo')}, sort_keys=True).encode()
    ).hexdigest()


def validate(a):
    if not isinstance(a, dict):
        raise ValueError('Nieprawidłowy wynik modelu.')
    score = a.get('match')
    if (
        ...
        raise ValueError('Nieprawidłowa ocena dopasowania.')
    for k in ('summary', 'visible', 'unknown', 'red_flags'):
        if k == 'summary':
            if not isinstance(a.get(k), str) or len(a[k]) > 3000:
                raise ValueError('Brak podsumowania modelu.')
        elif (
            not isinstance(a.get(k), list)
            or len(a[k]) > 20
            or any(not isinstance(x, str) or len(x) > 1500 for x in a[k])
        ):
            raise ValueError('Nieprawidłowe uzasadnienie modelu.')
    return a
...
SCHEMA = {
    ...
    'properties': {
        ...
        'summary': {'type': 'string'},
        **{k: {'type': 'array', 'items': {'type': 'string'}} for k in ('visible', 'unknown', 'red_flags')},
    },
    'required': ['match', 'summary', 'visible', 'unknown', 'red_flags'],
...
def analyze(key, criteria):
    if not isinstance(criteria, str) or not 5 <= len(criteria.strip()) <= 2000:
        raise ValueError('Opisz szukane miejsce: 5–2000 znaków.')
    criteria = criteria.strip()
    if not LOCK.acquire(blocking=False):
        raise ValueError('Trwa już analiza miejsca. Poczekaj na jej zakończenie.')
    try:
        if not status()['ready']:
            raise ValueError(status()['message'])
        p = store.get_place(key)
        digest = fingerprint(p)
        old = p.get('local_match')
        if (
            old
            and old.get('criteria') == criteria
            and old.get('fingerprint') == digest
            and old.get('model') == MODEL
            and old.get('prompt_version') == 2
        ):
            return old
        images = []
        used = []
        ...
        for ph in p.get('photos', [])[:3]:
            try:
                images.append(image_bytes(ph['url']))
                used.append(ph['url'])
            except Exception as e:
                skipped.append(str(e))
        if not images:
            raise ValueError(
                'Brak zdjęć możliwych do analizy. ' + (' '.join(skipped[:1]) or 'Zaimportuj zdjęcia miejsca.')
            )
        ...
        payload = dict(
            ...
            stream=False,
            format=SCHEMA,
            options={'temperature': 0, 'num_predict': 1100, 'num_ctx': 8192},
            messages=[
                dict(role='system', content=system),
                dict(
                    role='user',
                    content='Oceń zdjęcia i odpowiedz po polsku. Wszystkie opisy w summary, visible, unknown i red_flags muszą być po polsku. Dane:\n'
                    + json.dumps(
                        {
                            'preferences': criteria,
                            'place': {k: p.get(k) for k in ('name', 'description', 'comments', 'geo')},
                            'schema': SCHEMA,
                        },
                        ensure_ascii=False,
                    )[:18000],
        ...
        if not response.get('done') or response.get('done_reason') == 'length':
            raise ValueError('Model nie ukończył oceny. Spróbuj ponownie.')
        ...
        with store.connect() as db:
            ...
            if current is None or fingerprint(current) != digest:
                raise ValueError('Dane miejsca zmieniły się podczas analizy; spróbuj ponownie.')
