...
from .p4n.endpoints import HOST as P4N_HOST
...
LAST = {}
LAYERS = {
    6: ('Miejsce biwakowania', 'camp_site'),
    7: ('Miejsce biwakowania', 'camp_site'),
    8: ('Pole biwakowe', 'camp_site'),
    9: ('Pole biwakowe', 'camp_site'),
    10: ('Kemping', 'camp_site'),
    11: ('Kemping', 'camp_site'),
    17: ('Parking leśny', 'parking'),
    18: ('Parking leśny', 'parking'),
    19: ('Miejsce postoju', 'parking'),
    20: ('Miejsce postoju', 'parking'),
    25: ('Punkt widokowy', 'viewpoint'),
    26: ('Punkt wodowania', 'water_access'),
}
SOURCES = [
    dict(
        id='osm',
    ...
    dict(
        id='bdl',
        name='Lasy Państwowe / Czas w Las',
        mode='automatic',
        status='Automatyczny import',
        url='https://www.bdl.lasy.gov.pl/portal/uslugi-ogc',
    ...
    dict(
        ...
        name='iOverlander',
        mode='file',
        status='Oficjalny eksport + folder importu',
        url='https://ioverlander.com/countries',
    ...
    dict(
        ...
        name='P4N',
        mode='public_web',
        status='Publiczna mapa · import próbki',
        url=P4N_HOST + '/en/cgu',
    ...
    dict(
        ...
        name='Campercontact',
        mode='external',
        status='Dostęp partnerski do ustalenia',
        url='https://www.campercontact.com/en/content/partner-with-campercontact',
    ...
    dict(
        ...
        name='camping.info',
        mode='external',
        status='Brak potwierdzonego otwartego feedu',
    ...
    dict(
        ...
        name='OpenCampingMap',
        mode='shared',
        status='Miejsca przez OpenStreetMap',
        url='https://opencampingmap.org/',
        note='Wspólna baza OSM, nie drugie niezależne źródło miejsc. Opinie OpenCampingMap nie są pobierane.',
    ),
    dict(
        id='files',
        name='Inne serwisy / własne punkty',
        mode='file',
        status='GPX · CSV · GeoJSON · JSON · ZIP',
        url='',
...
def request_json(url, params=None, post=False, provider='bdl', ttl=86400):
    ...
    with LOCK:
        ...
        if pause > 0:
            raise ValueError(f'Źródło ograniczyło ruch. Ponów za {math.ceil(pause)} s; nie obchodzimy limitu.')
        time.sleep(max(0, (5 if provider == 'osm' else 0.25) - (time.monotonic() - LAST.get(provider, 0))))
        store.allowance(provider)
        req = urllib.request.Request(
            url if post else url + ('?' + encoded.decode() if encoded else ''),
        ...
        try:
            with urllib.request.urlopen(req, timeout=50) as r:
                ...
                if len(data) > 8_000_000:
                    raise ValueError('Odpowiedź przekracza 8 MB; zmniejsz obszar.')
            store.account_bytes(provider, len(data))
            result = json.loads(data)
            if result.get('error'):
                raise ValueError('Źródło zwróciło błąd: ' + str(result['error'].get('message', result['error'])))
            if result.get('remark'):
                raise ValueError('Niepełna odpowiedź Overpass: ' + result['remark'])
            store.cache_put(key, result)
            return result, False
        except urllib.error.HTTPError as e:
            if e.code in (429, 406, 503, 504):
                try:
                    delay = max(60, min(3600, int(e.headers.get('Retry-After', '60'))))
                except ValueError:
                    delay = 60
            ...
            raise ValueError(f'{provider}: brak odpowiedzi. Spróbuj później; cache i lokalna baza są zachowane.')
        finally:
            LAST[provider] = time.monotonic()
...
def check_area(a):
    ...
    if size > 30000 or e - w > 6 or n - s > 4:
        raise ValueError(
            'Pojedynczy obszar importu: maks. 30 000 km², 6° szerokości i 4° wysokości. Podziel region na mniejsze bbox.'
        )
...
def osm_spot(e, stamp):
    ...
    labels = {
        'camp_site': 'Biwak / kemping',
        'camp_pitch': 'Stanowisko namiotowe',
        'caravan_site': 'Miejsce dla kamperów',
        'viewpoint': 'Punkt widokowy',
        'picnic_site': 'Miejsce piknikowe',
        'wilderness_hut': 'Schron terenowy',
        'beach': 'Plaża',
        'parking': 'Parking',
    }
    ident = f'{e["type"]}/{e["id"]}'
    url = 'https://www.openstreetmap.org/' + ident
    ...
    if 'surface' in t:
        geo['surface'] = t['surface']
    try:
        if 'ele' in t:
            geo['elevation_m'] = float(t['ele'].replace(' m', '').replace(',', '.'))
    except ValueError:
        pass
    if kind == 'viewpoint':
        geo['viewpoint'] = True
    ...
    for topic, value in [('motorcycle', motor), ('tent', t.get('tents', t.get('camping')))]:
        if value in ('yes', 'designated', 'no', 'private'):
            ev.append(
                dict(
                    topic=topic,
                    value='forbidden' if value == 'no' else 'unknown' if value == 'private' else 'allowed',
                    authority='community',
                    date=None,
                    source_url=url,
                    text=f'OSM {topic}={value}. Tag społeczności; data pobrania nie jest datą potwierdzenia.',
                )
            )
    photos = []
    if valid_url(t.get('image', '')):
        photos.append(
            dict(url=t['image'], caption='Zdjęcie wskazane w OSM; prawa i autor na stronie źródłowej', source_url=url)
        )
    commons = t.get('wikimedia_commons', '')
    p = dict(
        id=ident,
        source='osm',
        name=t.get('name:pl') or t.get('name') or labels.get(kind, kind) + ' · OSM ' + str(e['id']),
        lat=c['lat'],
        lon=c['lon'],
        type=kind,
        ...
        source_date=None,
        raw=e,
        extra_flags=['Punkt widokowy/piknik/plaża nie jest potwierdzonym noclegiem']
        if kind in ('viewpoint', 'picnic_site', 'beach', 'wilderness_hut')
        else [],
        commons_file=commons if commons.startswith('File:') else '',
    )
    if e['type'] != 'node':
        p['extra_flags'].append('Współrzędne środka obiektu OSM — nie zweryfikowanego wjazdu')
    return p


def fetch_osm(a, progress):
    check_area(a)
    progress('Pobieram miejsca OpenStreetMap…')
    raw, cached = request_json(OVERPASS, {'data': osm_query(a)}, post=True, provider='osm')
    stamp = now()
    places = [p for e in raw.get('elements', []) if (p := osm_spot(e, stamp)) and inside(p, a)]
    ...
    return normalized, dict(
        ...
        cached=cached,
        status='ok',
        coverage='Wszystkie rekordy odpowiedzi dla wybranych tagów i punktów reprezentatywnych w obszarze. Baza społeczności może mieć braki.',
        source_date=raw.get('osm3s', {}).get('timestamp_osm_base'),
    )
...
def bdl_spot(feature, layer, stamp):
    ...
    point = representative(feature.get('geometry', {}))
    if not point:
        return None
    title, kind = LAYERS[layer]
    ident = str(attrs.get('foreign_key') or f'{layer}/{attrs["objectid"]}')
    source_url = (
        BDL
        + f'/{layer}/query?'
        + urllib.parse.urlencode(dict(f='pjson', objectIds=attrs['objectid'], outFields='*', outSR=4326))
    )
    fields = {
        'adres': 'Adres',
        'rec_od_do': 'Dostępność',
        'metod_rez': 'Rezerwacja',
        'uwagi': 'Uwagi',
        'infr_tow': 'Otoczenie',
        'inne_atr': 'Atrakcje',
        'otw_od': 'Otwarte od',
        'otw_do': 'Otwarte do',
        'nocleg': 'Nocleg (T/N)',
        'koszt_mnam': 'Koszt małego namiotu',
    }
    desc = '\n'.join(
        f'{label}: {clean(attrs[k])}' for k, label in fields.items() if attrs.get(k) not in (None, '', 'brak')
    )
    source_date = None
    if attrs.get('edit_time'):
        try:
            source_date = dt.datetime.fromtimestamp(attrs['edit_time'] / 1000, dt.timezone.utc).isoformat()
        except (ValueError, OverflowError):
            pass
    geo = {'forest': True, 'source_url': source_url, 'date': source_date[:10] if source_date else None}
    if kind == 'viewpoint' or attrs.get('pkt_wid') == 'T':
        geo['viewpoint'] = True
    if kind == 'water_access':
        geo['water_access_point'] = True
    ...
    if 'rings' in feature.get('geometry', {}):
        flags.append('Środek obszaru BDL — sprawdź granice i rzeczywisty wjazd')
    p = dict(
        id=ident,
        source='bdl',
        name=clean(attrs.get('nzw_ob')) or title + ' · ' + str(attrs['objectid']),
        lat=point[0],
        lon=point[1],
        type=kind,
        description=desc,
        ...
        license='Bank Danych o Lasach / Lasy Państwowe · regulamin BDL, rozdział V',
        raw=feature,
        extra_flags=flags,
    )
...
def fetch_bdl(a, progress):
    ...
    for layer, (title, kind) in LAYERS.items():
        try:
            ...
            if 'objectid' not in available:
                raise ValueError('Zmienił się schemat warstwy BDL; wymagana aktualizacja adaptera.')
            total = 0
            offset = 0
            cached_all = True
            seen = set()
            while True:
                ...
                params = dict(
                    f='json',
                    where='1=1',
                    geometry=','.join(str(x) for x in a['bbox']),
                ...
                if any(i in seen for i in ids):
                    raise ValueError('Powtórzona strona odpowiedzi; import warstwy niepełny.')
                seen.update(ids)
                for f in features:
                ...
                if not raw.get('exceededTransferLimit'):
                    break
                if not features or offset >= 4750:
                    raise ValueError('Limit/niepełna paginacja. Zmniejsz obszar.')
                offset += len(features)
            reports.append(dict(layer=layer, status='ok', count=total, cached=cached_all))
        except ValueError as e:
            reports.append(dict(layer=layer, status='error', message=str(e)))
    normalized, dups = normalize({'spots': allspots})
    return normalized, dict(
        provider='bdl',
        count=len(normalized),
        status='partial' if any(r['status'] == 'error' for r in reports) else 'ok',
        layers=reports,
        coverage='12 wybranych warstw BDL, w obszarze według punktów reprezentatywnych; poligony nie są trasą dojazdu.',
        duplicates=dups,
    )
...
def distance_segment(lat, lon, a, b):
    ...
    ax = (a['lon'] - lon) * scale * cs
    ay = (a['lat'] - lat) * scale
    bx = (b['lon'] - lon) * scale * cs
    by = (b['lat'] - lat) * scale
    vx, vy = bx - ax, by - ay
    t = max(0, min(1, -(ax * vx + ay * vy) / (vx * vx + vy * vy))) if vx or vy else 0
...
def enrich(p):
    ...
    water = sorted(
        [
            (d, e)
            for d, e in nearest
            if e.get('tags', {}).get('waterway') or e.get('tags', {}).get('natural') == 'water'
        ],
        key=lambda x: x[0],
    )
    roads = sorted([(d, e) for d, e in nearest if e.get('tags', {}).get('highway')], key=lambda x: x[0])
    p['geo']['context_fetched_at'] = now()
    p['geo']['context_source'] = 'OpenStreetMap / Overpass'
    if water:
        d, e = water[0]
        p['geo'].update(
            water_distance_m=round(d),
            water_kind=e['tags'].get('waterway', e['tags'].get('water', 'water')),
            water_source_url='https://www.openstreetmap.org/way/' + str(e['id']),
        )
    if roads:
        d, e = roads[0]
        p['geo'].update(
            nearest_road_m=round(d),
            nearest_road_tags=e.get('tags', {}),
            nearest_road_url='https://www.openstreetmap.org/way/' + str(e['id']),
        )
    p.setdefault('extra_flags', [])
    note = 'Kontekst OSM: odległość do geometrii wody/drogi, nie zweryfikowana trasa ani prawo dostępu.'
    if note not in p['extra_flags']:
        p['extra_flags'].append(note)
    if not water:
        p['geo']['water_search_note'] = 'Nie znaleziono geometrii wody w zapytaniu 1,2 km. Nie dowodzi to braku wody.'
    p.pop('saved_ai', None)
    return p
